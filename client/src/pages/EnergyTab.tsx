    ...
    if (isLoading && !data) {
        return (
            <div className="space-y-6">
                ...
                <div className="glass-panel rounded-2xl h-[360px] skeleton" />
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                {/* Empire total */}
                <div className="glass-panel-interactive rounded-xl p-4 flex flex-col col-span-2 lg:col-span-1">
                    <p className="text-sm opacity-70 text-[var(--text-muted)]">🏛️ Empire Total</p>
                    <p className="text-2xl font-outfit font-bold tracking-tight mt-auto text-glow" style={{ color: EMPIRE_COLOR, textShadow: `0 0 20px ${EMPIRE_COLOR}40` }}>
                        {latestEmpire !== null ? formatEnergy(latestEmpire) : "—"}
                    </p>
                </div>

                {/* Per-room cards */}
                {roomSummaries.map((r) => (
                    <div key={r.room} className="glass-panel-interactive rounded-xl p-4 flex flex-col">
                        <p className="text-sm opacity-70 text-[var(--text-muted)]">🏠 {r.room}</p>
                        <p className="text-2xl font-outfit font-bold tracking-tight mt-auto" style={{ color: r.color, textShadow: `0 0 20px ${r.color}40` }}>
                            {formatEnergy(r.value)}
                        </p>
                    </div>
                ))}
            </div>
